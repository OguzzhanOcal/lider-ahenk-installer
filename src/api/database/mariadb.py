...
class MariaDbInstaller(object):
    ...
    def __init__(self, ssh_api):
        ...
        self.cmd_pwd = "echo {0} | sudo -S su && echo \'mariadb-server mysql-server/root_password password {1}\'| sudo debconf-set-selections"
        self.cmd_pwd_again = "echo {0} | sudo -S su && echo \'mariadb-server mysql-server/root_password_again password {1}\'| sudo debconf-set-selections"
        # sudo apt-get install -qq mariadb-server
        self.cmd_db_install = "echo {0} | sudo -S apt install -y --force-yes mariadb-server"
